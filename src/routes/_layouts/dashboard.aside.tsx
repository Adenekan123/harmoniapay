    ...
    return sideNavs.map((nav) => {
      ...
      return (
        <Link to={path} key={title} className={`flex gap-6 items-center py-4 px-8 group` }>
          <span>
            <Icon size={24} className={pathname.includes(path)? 'text-secondary':'text-secondarytint group-hover:text-secondary'} />
          </span>
          <span className="text-white group-hover:text-secondary">{title}</span>
        </Link>
      );
    });
  }, [pathname]);
